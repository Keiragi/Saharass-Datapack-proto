...
# 視線先の判定
  ...
  run function saharass:skill/all/07/detect

# tagのついたエンティティを計測 + いなければリセット
  execute store result score $Hit Inferno_Count if entity @e[type=#saharass:enemy,tag=Inferno,distance=..16]
  scoreboard players operation @s Inferno_Count += $Hit Inferno_Count
  execute unless entity @e[type=#saharass:enemy,tag=Inferno,distance=..16] run scoreboard players set @s Inferno_Count 0
...
# 攻撃数のカウント
  ...
  execute if score @s Inferno_Count matches 9.. run function saharass:skill/all/06/fire/high {"icon":"✦ ","hunger":"79","number":"7"}
