...
# サブかスイッチで無ければメイン、スペシャルの処理
  ...
  run function saharass:core/inventory/changed/used_item/main with storage temp: Item

# スクロールはitem modifierなので別で実行
  execute if data storage temp: Item{"Saharass":"Sub"} unless data storage temp: Item{"Number":"00"} \
  run function saharass:core/inventory/changed/used_item/scroll/sub with storage temp: Item
  execute if data storage temp: Item{"Saharass":"Switch"} \
  run function saharass:core/inventory/changed/used_item/scroll/switch with storage temp: Item
