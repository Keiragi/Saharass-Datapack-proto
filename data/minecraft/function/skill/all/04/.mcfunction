...
# 十字上の相手に実行
  ...
  execute positioned ~-8 ~-2 ~-0.5 as @e[type=#enemy,dx=15,dy=4] run tag @s add Sacred_Fire
  execute positioned ~-0.5 ~-2 ~-8 as @e[type=#enemy,dz=15,dy=4] run tag @s add Sacred_Fire
  execute as @e[type=#enemy,tag=Sacred_Fire] run function skill/all/04/damage with storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].Skill

# 1秒後にダメージ判定
  schedule function skill/all/04/burn/ 1s replace
