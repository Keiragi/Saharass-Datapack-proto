# 演出
  playsound item.firecharge.use player @a[distance=..16] ~ ~1 ~ 0.5 1
...
# 火の玉召喚 + データセット
  execute anchored eyes positioned ^ ^ ^ summon fireball run function skill/all/02/set_fireball_data with storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].Skill

# 1秒後に消滅
  schedule function skill/all/02/schedule 1s append
