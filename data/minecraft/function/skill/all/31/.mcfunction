## アース
  # 周りにいなければ中止処理
    execute unless entity @e[type=#enemy,distance=..10,predicate=flags/on_ground] \
  ...
  # 10m以内の敵にスキルの処理
    execute as @e[type=#enemy,distance=..10,predicate=flags/on_ground] at @s \
    run function skill/all/31/damage with storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].Skill
